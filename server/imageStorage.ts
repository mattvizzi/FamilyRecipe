...
/**
 * Upload a base64 profile image to Object Storage and return the public URL path.
 * 
 * @param base64Data - Base64 encoded image data (with or without data URL prefix)
...
export async function uploadProfileImage(
  ...
  // Get the public directory for storing profile images
  const publicPaths = objectStorageService.getPublicObjectSearchPaths();
  const publicDir = publicPaths[0];
  
  // Generate unique filename
  const objectId = `profile-images/${userId}-${randomUUID()}.${extension}`;
  const fullPath = `${publicDir}/${objectId}`;
